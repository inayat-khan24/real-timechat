    ...
    <div className="min-h-screen bg-gray-100 p-6 flex justify-center">
      <div className="w-full max-w-6xl h-[80vh] bg-white rounded-xl shadow-xl flex flex-col lg:flex-row overflow-hidden">
        ...
        <div className="w-full lg:w-1/3 border-r overflow-y-auto p-4">
          <div className="flex justify-between items-center mb-4">
            ...
            <select
              value={theme}
              onChange={(e) => setTheme(e.target.value)}
              className="hidden appearance-none focus:outline-none focus:ring-0 border-none lg:block px-2 py-1 rounded-md border text-sm"
            >
          ...
          {allUsers.filter(u => u.username !== username).map((user, idx) => (
            <button
              key={idx}
              onClick={() => {
              ...
              className={`flex items-center gap-3 p-2 rounded-lg w-full text-left ${
                selectedUser === user.username ? "bg-blue-200" : "hover:bg-gray-100"
              }`}
            >
              <img src={`http://localhost:5000/uploads/${user.profilePic}`} className="h-8 w-8 rounded-full" alt="pf" />
        ...
        <div className="w-full lg:w-2/3 flex flex-col">
          ...
          {/* Messages */}
          <div className={`flex-1 p-4 overflow-y-auto space-y-4 transition-all duration-300 ${themeClasses[theme]}`}>
            {privateChat.map((msg, idx) => {
              const isMe = msg.sender === username;
              return (
                <div key={idx} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-xs p-3 rounded-md shadow ${
          ...
          {selectedUser && (
            <div className="p-4 border-t bg-white flex items-center gap-2 relative">
              ...
              <button onClick={sendPrivateMessage} className="bg-blue-500 text-white px-4 py-2 rounded-md">
                Send
              </button>
